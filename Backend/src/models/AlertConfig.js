...
// `low_stock` + 1 for `quotation_request`. Seeded idempotently at startup (see
// utils/autoSeed.js) so the Settings UI always edits known rows.
const alertConfigSchema = new mongoose.Schema({
  group: { type: String, enum: ['design', 'sales_approval', 'operations_approval', 'task', 'dispatch_reason', 'dispatch_status', 'lr_payment', 'low_stock', 'quotation_request'], required: true },
