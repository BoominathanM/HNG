...
// `dispatch_reason`. Seeded idempotently at startup (see utils/autoSeed.js) so the
// Settings UI always edits known rows.
const alertConfigSchema = new mongoose.Schema({
  group: { type: String, enum: ['design', 'sales_approval', 'operations_approval', 'task', 'dispatch_reason'], required: true },
